export const LANGS = ["kk", "ru", "en"];

export const dict = {
  kk: {
    nav: {
      about: "Біз туралы",
      association: "Қауымдастық",
      members: "Мүшелер",
      activities: "Қызмет бағыттары",
      reports: "Талдамалық есептер",
      pressCenter: "Баспасөз орталығы",
      media: "Медиа",
      partners: "Серіктестер",
      events: "Іс-шаралар",
      ner: "НЭД",
      exia: "EXIA",
      contact: "Байланыс",
      memberLogin: "Мүше кабинеті",
      join: "Қосылу",
    },
    association: {
      about: "Біз туралы",
      members: "Қауымдастық мүшелері",
      benefits: "Мүшеліктің артықшылықтары",
      conditions: "Қосылу шарттары",
      structure: "Құрылым",
      career: "Мансап",
      procurement: "Сатып алулар",
      contacts: "Байланыс",
    },
    activities: {
      eyebrow: "ҚЫЗМЕТ БАҒЫТТАРЫ",
      title: "Біздің қызмет",
      subtitle: "KAZENERGY 13 стратегиялық бағытта жұмыс істейді — энергетиканың құқықтық негізінен бастап адами капиталға дейін.",
      items: {
        oilGas: "Мұнай және газ",
        power: "Электр энергетикасы",
        finance: "Қаржы-экономика",
        ecology: "Экология",
        regulation: "Техникалық реттеу",
        legal: "Құқықтық сүйемелдеу",
        intl: "Халықаралық ынтымақтастық",
        capital: "Адами капитал",
        forum: "KAZENERGY форумы",
        ner: "Ұлттық энергетикалық есеп",
        councils: "Координациялық кеңестер",
        education: "Білім беру бағдарламасы",
        science: "Ғылыми-техникалық кеңес",
      },
    },
    press: {
      news: "Жаңалықтар",
      magazine: "Журнал",
      releases: "Баспасөз релиздері",
      photo: "Фотогалерея",
      video: "Видеогалерея",
      anniversary: "KAZENERGY 20-жылдығы",
      victory: "Ұлы Жеңіске 80 жыл",
    },
    members: {
      production: "Өндіру компаниялары",
      services: "Сервистік компаниялар",
      refining: "Мұнай өңдеу",
      renewable: "Жаңартылатын энергия",
    },
    reports: {
      annual: "Жылдық есеп",
      esg: "ESG шолуы",
      market: "Нарық перспективасы",
      outlook: "Энергия болжамы",
    },
    media: {
      magazine: "Журнал",
      news: "Жаңалықтар",
      press: "Баспасөз релиздері",
      gallery: "Галерея",
    },
    cta: {
      search: "Іздеу...",
      readOnline: "Онлайн оқу",
      downloadPDF: "PDF жүктеу",
      learnMore: "Толығырақ",
      becomeMember: "Мүше болу",
      viewReport: "Есепті көру",
      readMore: "Толығырақ оқу",
      allNews: "Барлық жаңалықтар",
      addToCalendar: "Күнтізбеге қосу",
      openReport: "НЭД-ті ашу",
    },
    hero: {
      eyebrow: "БІРЛІК ЭНЕРГИЯСЫ · 20 ЖЫЛ КӨШБАСШЫЛЫҚ",
      title1: "Қазақстанның дамуын",
      title2: "20 жыл бойы отандырып келеміз",
      subtitle:
        "KAZENERGY Қауымдастығы — мұнай-газ және энергетика саласындағы үкімет, бизнес және халықаралық қауымдастықтың сенімді көпірі.",
      stat1: "Мүше компания",
      stat2: "Дипломатиялық серіктес",
      stat3: "Жариялаған есептер",
      stat4: "Зерттеу жобалары",
    },
    industry: {
      eyebrow: "ҚАУЫМДАСТЫҚ МҮШЕЛЕРІНІҢ ҮЛЕСІ",
      title: "Қазақстанның энергетикасы — KAZENERGY-де",
      subtitle:
        "Біздің мүше компаниялар ел экономикасының энергетикалық тірегін құрайды.",
      items: {
        oil: "Мұнай мен конденсат өндіру",
        gas: "Газ өндіру (табиғи және СНГ)",
        oilTransport: "Мұнайды тасымалдау",
        gasTransport: "Газды тасымалдау",
        powerGen: "Электр энергиясын өндіру",
        gridCapacity: "БЭЖ қондырғылы қуат үлесі",
        transmission: "Ұлттық электр желісі арқылы беру",
        refining: "Мұнай өңдеу",
        coal: "Көмір өндіру",
      },
    },
    knowledge: {
      eyebrow: "БІЛІМ ОРТАЛЫҒЫ",
      title: "Ақпараттық-сараптамалық журнал",
      subtitle:
        "KAZENERGY журналы — үш тілде жарияланатын стратегиялық сараптама, нарық деректері және сұхбаттар.",
      issue: "Шығарылым",
    },
    esg: {
      eyebrow: "ESG · ЭНЕРГИЯ КӨРСЕТКІШТЕРІ",
      title: "Тірі көрсеткіштер тақтасы",
      subtitle:
        "Қазақстанның энергетикалық трансформациясының нақты уақыттағы көрінісі.",
      cards: {
        oil: "Мұнай өндіру (мың барр./күн)",
        renewable: "ЖЭК үлесі",
        carbon: "Көміртек ізін азайту",
        gas: "Табиғи газ экспорты",
      },
    },
    news: {
      eyebrow: "БАСПАСӨЗ ОРТАЛЫҒЫ",
      title: "Соңғы жаңалықтар",
      subtitle: "Қауымдастықтың және сала жаңалықтарынан хабардар болыңыз.",
      items: [
        {
          date: "2 сәуір 2026",
          tag: "Форум",
          title:
            "KAZENERGY GEOSCIENCE & EXPLORATION CENTRAL ASIA форумында",
          excerpt:
            "Орталық Азияның алғашқы Халықаралық геологиялық форумы Астанада өз жұмысын бастады.",
        },
        {
          date: "2 наурыз 2026",
          tag: "HR",
          title:
            "Қазатомпром әйелдер көшбасшылығы туралы воркшоп өткізді",
          excerpt:
            "Атом саласындағы тең мүмкіндіктер мәдениетіне арналған стратегиялық сессия.",
        },
        {
          date: "27 ақпан 2026",
          tag: "Технологиялар",
          title:
            "Заман сын-тегеуріндері: технологиялар мен ИИ дәуіріндегі қауіпсіздік",
          excerpt:
            "«Ембімұнайгаз» АҚ-та сала көшбасшылары мен сарапшыларға арналған форум өтті.",
        },
      ],
    },
    events: {
      eyebrow: "ІС-ШАРАЛАР КҮНТІЗБЕСІ",
      title: "Алдағы оқиғалар",
      subtitle: "KAZENERGY қатысатын халықаралық сала іс-шаралары.",
      items: [
        {
          date: "8–10 сәуір 2026",
          city: "Атырау",
          title: "Atyrau Oil & Gas 2026",
          tag: "Көрме",
        },
        {
          date: "28–30 сәуір 2026",
          city: "Алматы",
          title: "14-ші Қазақстан Еңбек қауіпсіздігі конференциясы",
          tag: "Конференция",
        },
        {
          date: "30 қыркүйек – 1 қазан 2026",
          city: "Алматы",
          title: "KIOGE 2026 — Қазақстан Мұнай-газ көрмесі",
          tag: "Көрме",
        },
        {
          date: "Қыркүйек 2026",
          city: "Астана",
          title: "Еуразиялық KAZENERGY форумы",
          tag: "Форум",
        },
      ],
    },
    ned: {
      eyebrow: "ҰЛТТЫҚ ЭНЕРГЕТИКАЛЫҚ ЕСЕП",
      title: "Қазақстанның энергетикасына стратегиялық көзқарас",
      subtitle:
        "НЭД — Қазақстанның мұнай-газ және энергетика саласының тұрақты дамуы туралы жетекші сараптамалық құжат. 2005 жылдан бері жыл сайын жарияланады.",
      stat1: "Сала көрсеткіші",
      stat2: "Сарапшы автор",
      stat3: "Тіл",
      cta: "НЭД 2025 жүктеу",
    },
    partners: {
      eyebrow: "ЖАҺАНДЫҚ СЕРІКТЕСТІК",
      title: "Сенім логотиптерінің қабырғасы",
      subtitle:
        "Біз ең сенімді халықаралық институттармен және энергетика алыптарымен жұмыс істейміз.",
    },
    footer: {
      tagline: "Энергетика — ел келешегінің тірегі.",
      rights: "Барлық құқықтар қорғалған.",
      privacy: "Құпиялық саясаты",
      terms: "Қызмет шарттары",
      sitemap: "Сайт картасы",
      accreditation: "Басқа ұйымдардағы аккредитация",
      address: "Астана қ. Қабанбай батыр д.17, Б блогы",
    },
    victoryBanner: {
      title: "Ұлы Жеңіске 80 жыл",
      subtitle: "1945–2025 · Үлкен Отан соғысындағы жеңіс мерейтойы",
      cta: "Бағдарлама",
    },
    footerCols: {
      association: "Қауымдастық",
      activities: "Қызмет",
      press: "Баспасөз",
      contact: "Байланыс",
    },
  },
  ru: {
    nav: {
      about: "О нас",
      association: "Ассоциация",
      members: "Участники",
      activities: "Деятельность",
      reports: "Аналитические отчёты",
      pressCenter: "Пресс-центр",
      media: "Медиа",
      partners: "Партнёры",
      events: "События",
      ner: "НЭД",
      exia: "EXIA",
      contact: "Контакты",
      memberLogin: "Кабинет участника",
      join: "Присоединиться",
    },
    association: {
      about: "О нас",
      members: "Члены Ассоциации",
      benefits: "Преимущества членства",
      conditions: "Условия вступления",
      structure: "Структура",
      career: "Карьера",
      procurement: "Закупки",
      contacts: "Контакты",
    },
    activities: {
      eyebrow: "НАША ДЕЯТЕЛЬНОСТЬ",
      title: "Стратегические направления",
      subtitle: "KAZENERGY работает в 13 стратегических направлениях — от правового регулирования энергетики до развития человеческого капитала.",
      items: {
        oilGas: "Нефть и газ",
        power: "Электроэнергетика",
        finance: "Финансово-экономическая деятельность",
        ecology: "Экология",
        regulation: "Техническое регулирование",
        legal: "Правовое сопровождение",
        intl: "Международное сотрудничество",
        capital: "Развитие человеческого капитала",
        forum: "Форум KAZENERGY",
        ner: "Национальный Энергетический Доклад",
        councils: "Координационные советы",
        education: "Образовательная программа",
        science: "Научно-технический совет",
      },
    },
    press: {
      news: "Новости",
      magazine: "Журнал",
      releases: "Пресс-релизы",
      photo: "Фотогалерея",
      video: "Видеогалерея",
      anniversary: "20-летие KAZENERGY",
      victory: "Ұлы жеңіске 80 жыл",
    },
    members: {
      production: "Добывающие компании",
      services: "Сервисные компании",
      refining: "Нефтепереработка",
      renewable: "Возобновляемая энергия",
    },
    reports: {
      annual: "Годовой отчёт",
      esg: "Обзор ESG",
      market: "Перспективы рынка",
      outlook: "Энергопрогноз",
    },
    media: {
      magazine: "Журнал",
      news: "Новости",
      press: "Пресс-релизы",
      gallery: "Галерея",
    },
    cta: {
      search: "Поиск...",
      readOnline: "Читать онлайн",
      downloadPDF: "Скачать PDF",
      learnMore: "Подробнее",
      becomeMember: "Стать участником",
      viewReport: "Открыть отчёт",
      readMore: "Читать далее",
      allNews: "Все новости",
      addToCalendar: "В календарь",
      openReport: "Открыть НЭД",
    },
    hero: {
      eyebrow: "ЭНЕРГИЯ ЕДИНСТВА · 20 ЛЕТ ЛИДЕРСТВА",
      title1: "Двадцать лет питаем",
      title2: "развитие Казахстана",
      subtitle:
        "Ассоциация KAZENERGY — надёжный мост между правительством, бизнесом и международным сообществом в нефтегазовой и энергетической сферах.",
      stat1: "Компаний-членов",
      stat2: "Дипломатических партнёров",
      stat3: "Опубликованных отчётов",
      stat4: "Исследовательских проектов",
    },
    industry: {
      eyebrow: "ДОЛЯ ЧЛЕНОВ АССОЦИАЦИИ",
      title: "Энергетика Казахстана — это KAZENERGY",
      subtitle:
        "Компании-члены Ассоциации формируют энергетический фундамент экономики страны.",
      items: {
        oil: "Добыча нефти и конденсата",
        gas: "Добыча газа (природный и СНГ)",
        oilTransport: "Транспортировка нефти",
        gasTransport: "Транспортировка газа",
        powerGen: "Производство электроэнергии",
        gridCapacity: "Доля установленной мощности ЕЭС РК",
        transmission: "Транспортировка по национальной сети",
        refining: "Переработка нефти",
        coal: "Добыча угля",
      },
    },
    knowledge: {
      eyebrow: "ЦЕНТР ЗНАНИЙ",
      title: "Информационно-аналитический журнал",
      subtitle:
        "Журнал KAZENERGY — стратегическая аналитика, рыночные данные и интервью на трёх языках.",
      issue: "Выпуск",
    },
    esg: {
      eyebrow: "ESG · ЭНЕРГОПОКАЗАТЕЛИ",
      title: "Живая панель показателей",
      subtitle:
        "Энергетическая трансформация Казахстана в режиме реального времени.",
      cards: {
        oil: "Добыча нефти (тыс. барр./сутки)",
        renewable: "Доля ВИЭ",
        carbon: "Снижение углеродного следа",
        gas: "Экспорт природного газа",
      },
    },
    news: {
      eyebrow: "ПРЕСС-ЦЕНТР",
      title: "Последние новости",
      subtitle: "Будьте в курсе событий Ассоциации и отрасли.",
      items: [
        {
          date: "2 апреля 2026",
          tag: "Форум",
          title:
            "KAZENERGY на форуме GEOSCIENCE & EXPLORATION CENTRAL ASIA",
          excerpt:
            "В Астане начал работу первый Международный геологический форум Центральной Азии.",
        },
        {
          date: "2 марта 2026",
          tag: "HR",
          title:
            "Казатомпром провёл воркшоп по женскому лидерству в атомной отрасли",
          excerpt:
            "Стратегическая сессия о культуре равных возможностей и развитии лидерства.",
        },
        {
          date: "27 февраля 2026",
          tag: "Технологии",
          title:
            "Вызовы времени: технологии, безопасность и человеческий капитал в эпоху ИИ",
          excerpt:
            "В офисе АО «Эмбамунайгаз» прошёл форум для отраслевых лидеров и экспертов.",
        },
      ],
    },
    events: {
      eyebrow: "КАЛЕНДАРЬ СОБЫТИЙ",
      title: "Ближайшие мероприятия",
      subtitle: "Главные международные события отрасли при участии KAZENERGY.",
      items: [
        {
          date: "8–10 апреля 2026",
          city: "Атырау",
          title: "Atyrau Oil & Gas 2026",
          tag: "Выставка",
        },
        {
          date: "28–30 апреля 2026",
          city: "Алматы",
          title:
            "14-я Казахстанская конференция по охране труда и промышленной безопасности",
          tag: "Конференция",
        },
        {
          date: "30 сентября – 1 октября 2026",
          city: "Алматы",
          title: "KIOGE 2026 — Казахстанская нефтегазовая выставка",
          tag: "Выставка",
        },
        {
          date: "Сентябрь 2026",
          city: "Астана",
          title: "Евразийский форум KAZENERGY",
          tag: "Форум",
        },
      ],
    },
    ned: {
      eyebrow: "НАЦИОНАЛЬНЫЙ ЭНЕРГЕТИЧЕСКИЙ ДОКЛАД",
      title: "Стратегический взгляд на энергетику Казахстана",
      subtitle:
        "НЭД — флагманский аналитический документ KAZENERGY о устойчивом развитии нефтегазовой и энергетической отрасли. Публикуется с 2005 года.",
      stat1: "Отраслевых показателей",
      stat2: "Экспертов-авторов",
      stat3: "Языка",
      cta: "Скачать НЭД 2025",
    },
    partners: {
      eyebrow: "ГЛОБАЛЬНОЕ ПАРТНЁРСТВО",
      title: "Стена доверия",
      subtitle:
        "Мы сотрудничаем с ведущими международными институтами и энергетическими мейджорами.",
    },
    footer: {
      tagline: "Энергия — опора будущего страны.",
      rights: "Все права защищены.",
      privacy: "Политика конфиденциальности",
      terms: "Условия использования",
      sitemap: "Карта сайта",
      accreditation: "Аккредитация в иных организациях",
      address: "г. Астана, пр. Кабанбай батыра 17, блок Б",
    },
    victoryBanner: {
      title: "80 лет Великой Победы",
      subtitle: "1945–2025 · Юбилей Победы в Великой Отечественной войне",
      cta: "Программа",
    },
    footerCols: {
      association: "Ассоциация",
      activities: "Деятельность",
      press: "Пресс-центр",
      contact: "Контакты",
    },
  },
  en: {
    nav: {
      about: "About",
      association: "Association",
      members: "Members",
      activities: "Activities",
      reports: "Analytic Reports",
      pressCenter: "Press Centre",
      media: "Media",
      partners: "Partners",
      events: "Events",
      ner: "NER",
      exia: "EXIA",
      contact: "Contact",
      memberLogin: "Member log-in",
      join: "Join",
    },
    association: {
      about: "About us",
      members: "Association members",
      benefits: "Membership benefits",
      conditions: "How to join",
      structure: "Structure",
      career: "Careers",
      procurement: "Procurement",
      contacts: "Contacts",
    },
    activities: {
      eyebrow: "OUR ACTIVITIES",
      title: "Strategic directions",
      subtitle: "KAZENERGY operates across 13 strategic directions — from energy law and regulation to human capital development.",
      items: {
        oilGas: "Oil & gas",
        power: "Electric power",
        finance: "Financial & economic activities",
        ecology: "Ecology",
        regulation: "Technical regulation",
        legal: "Legal support",
        intl: "International cooperation",
        capital: "Human capital development",
        forum: "KAZENERGY Forum",
        ner: "National Energy Report",
        councils: "Coordination councils",
        education: "Educational programme",
        science: "Scientific & technical council",
      },
    },
    press: {
      news: "News",
      magazine: "Magazine",
      releases: "Press releases",
      photo: "Photo gallery",
      video: "Video gallery",
      anniversary: "20 years of KAZENERGY",
      victory: "80 years of the Great Victory",
    },
    members: {
      production: "Production Companies",
      services: "Oilfield Services",
      refining: "Downstream & Refining",
      renewable: "Renewable Energy",
    },
    reports: {
      annual: "Annual Report",
      esg: "ESG Review",
      market: "Market Outlook",
      outlook: "Energy Outlook",
    },
    media: {
      magazine: "Magazine",
      news: "Newsroom",
      press: "Press Releases",
      gallery: "Gallery",
    },
    cta: {
      search: "Search…",
      readOnline: "Read Online",
      downloadPDF: "Download PDF",
      learnMore: "Learn more",
      becomeMember: "Become a member",
      viewReport: "Open report",
      readMore: "Read more",
      allNews: "All news",
      addToCalendar: "Add to calendar",
      openReport: "Open NER",
    },
    hero: {
      eyebrow: "ENERGY OF UNITY · 20 YEARS OF LEADERSHIP",
      title1: "Two decades of fueling",
      title2: "Kazakhstan's progress",
      subtitle:
        "KAZENERGY Association is the trusted bridge between government, industry, and the international community across oil, gas, and the energy transition.",
      stat1: "Member companies",
      stat2: "Diplomatic partners",
      stat3: "Reports published",
      stat4: "Research initiatives",
    },
    industry: {
      eyebrow: "MEMBER COMPANIES' SHARE",
      title: "Kazakhstan's energy is KAZENERGY",
      subtitle:
        "Our member companies form the energy backbone of the national economy.",
      items: {
        oil: "Oil & condensate production",
        gas: "Gas production (natural & LNG)",
        oilTransport: "Oil transportation",
        gasTransport: "Gas transportation",
        powerGen: "Electricity generation",
        gridCapacity: "Installed capacity of the unified grid",
        transmission: "Transmission via the national grid",
        refining: "Oil refining",
        coal: "Coal production",
      },
    },
    knowledge: {
      eyebrow: "KNOWLEDGE CENTER",
      title: "Information-Analytical Magazine",
      subtitle:
        "The KAZENERGY magazine delivers strategic analysis, market data, and senior-leadership interviews across three languages.",
      issue: "Issue",
    },
    esg: {
      eyebrow: "ESG · ENERGY METRICS",
      title: "Live performance dashboard",
      subtitle:
        "A real-time view of Kazakhstan's energy transformation, indexed to global benchmarks.",
      cards: {
        oil: "Oil Production (k bbl/day)",
        renewable: "Renewable Share",
        carbon: "Carbon Footprint Reduction",
        gas: "Natural Gas Exports",
      },
    },
    news: {
      eyebrow: "PRESS CENTRE",
      title: "Latest news",
      subtitle: "Stay informed on Association initiatives and industry milestones.",
      items: [
        {
          date: "2 April 2026",
          tag: "Forum",
          title:
            "KAZENERGY at the GEOSCIENCE & EXPLORATION CENTRAL ASIA forum",
          excerpt:
            "Central Asia's first International Geological Forum opened in Astana with KAZENERGY in attendance.",
        },
        {
          date: "2 March 2026",
          tag: "HR",
          title:
            "Kazatomprom hosts a women-leadership workshop in the nuclear sector",
          excerpt:
            "A strategic session on equal-opportunity culture and leadership development across the industry.",
        },
        {
          date: "27 February 2026",
          tag: "Technology",
          title:
            "Challenges of the time: technology, safety and human capital in the AI era",
          excerpt:
            "Embamunaigas hosted an industry forum bringing together executives and senior experts.",
        },
      ],
    },
    events: {
      eyebrow: "EVENTS CALENDAR",
      title: "Upcoming events",
      subtitle: "Major industry events with KAZENERGY participation.",
      items: [
        {
          date: "8–10 April 2026",
          city: "Atyrau",
          title: "Atyrau Oil & Gas 2026",
          tag: "Exhibition",
        },
        {
          date: "28–30 April 2026",
          city: "Almaty",
          title: "14th Kazakhstan Conference on OHS & Industrial Safety",
          tag: "Conference",
        },
        {
          date: "30 Sep – 1 Oct 2026",
          city: "Almaty",
          title: "KIOGE 2026 — Kazakhstan Oil & Gas Exhibition",
          tag: "Exhibition",
        },
        {
          date: "September 2026",
          city: "Astana",
          title: "Eurasian KAZENERGY Forum",
          tag: "Forum",
        },
      ],
    },
    ned: {
      eyebrow: "NATIONAL ENERGY REPORT",
      title: "A strategic outlook on Kazakhstan's energy",
      subtitle:
        "The NER is KAZENERGY's flagship analytical publication on the sustainable development of the oil-and-gas and energy sector. Published annually since 2005.",
      stat1: "Industry indicators",
      stat2: "Contributing experts",
      stat3: "Languages",
      cta: "Download NER 2025",
    },
    partners: {
      eyebrow: "GLOBAL PARTNERSHIPS",
      title: "Wall of Trust",
      subtitle:
        "We collaborate with the world's leading institutions and energy majors to advance shared goals.",
    },
    footer: {
      tagline: "Energy is the foundation of a nation's future.",
      rights: "All rights reserved.",
      privacy: "Privacy Policy",
      terms: "Terms of Service",
      sitemap: "Sitemap",
      accreditation: "Accreditation in other organisations",
      address: "17 Kabanbai batyr Avenue, Block B, Astana, Kazakhstan",
    },
    victoryBanner: {
      title: "80 Years of the Great Victory",
      subtitle: "1945–2025 · Commemorating the anniversary of the Great Patriotic War",
      cta: "Programme",
    },
    footerCols: {
      association: "Association",
      activities: "Activities",
      press: "Press Centre",
      contact: "Contacts",
    },
  },
};

export const langLabel = {
  kk: "ҚАЗ",
  ru: "РУС",
  en: "ENG",
};
